...
public class SubscriptionController {
    ...
    @GetMapping
    public ApiResponse<PageResponse<SubscriptionItem>> list(@RequestParam(value = "keyword", required = false) String keyword) {
        return ApiResponse.ok(service.list(keyword));
    }

    @GetMapping("/{id}")
    public ApiResponse<SubscriptionItem> get(@PathVariable Long id) {
